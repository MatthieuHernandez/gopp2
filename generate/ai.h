...
#line 135 "../src/ai.h2"
    auto Ai::train(cpp2::impl::in<cpp2::i16> gobanSize) & -> void{
        if (cpp2::impl::cmp_greater(CPP2_UFCS(ssize)(inputs),50)) {// Only train on "short" game.
            return ; 
        }
        if (randomness == 1) {// Only process on "accurate" game.
            if (hasWon) {
                (*cpp2::impl::assert_not_null(optimizer)).learningRate *= learningRateUpdateRatio;
            }else {
                (*cpp2::impl::assert_not_null(optimizer)).learningRate /= CPP2_ASSERT_NOT_ZERO(CPP2_TYPEOF((*cpp2::impl::assert_not_null(optimizer)).learningRate),learningRateUpdateRatio);
            }
        }
...
#line 176 "../src/ai.h2"
    [[nodiscard]] auto Ai::calculateWinrate() const& -> float{
        return sumOfLastGameWon / CPP2_ASSERT_NOT_ZERO(CPP2_TYPEOF(sumOfLastGameWon),(float)(CPP2_UFCS(size)(lastGameWon))) * 100.0f; 
    }
...
#line 192 "../src/ai.h2"
    auto Ai::processEndGame() -> void{
        ...
        if (cpp2::impl::cmp_greater(CPP2_UFCS(ssize)(lastGameWon),100)) {
            setNextMessage("AI winrate: " + std::to_string(calculateWinrate()) + " %");
        }else {
            setNextMessage("AI winrate: ...");
        }
        setNextMessage("AI learning rate: " + std::to_string((*cpp2::impl::assert_not_null(optimizer)).learningRate) + " %");
    }
#endif
